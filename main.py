...
def show_music_progress(player):
    """
    Affiche une barre de progression en temps réel pendant la lecture.
    BUG CORRIGÉ : on surveille aussi l'état VLC pour détecter un changement
    de piste automatique (fin de piste → suivante) sans casser la boucle.
    """
    print()
    last_track = player.get_current_track_name()

    while True:
        # Vérifie si la piste a changé (fin automatique → suivante)
        current_track = player.get_current_track_name()
        if current_track != last_track:
            last_track = current_track
            print()  # saut de ligne propre avant la nouvelle piste

        # Attend que VLC soit vraiment en lecture (transition entre pistes)
        if not player.is_playing():
            time.sleep(0.3)
            # Si toujours pas en lecture après l'attente → la lecture est finie
            if not player.is_playing():
                break
            continue
...
def generic_player_menu(player, player_type: str):
    ...
    BUG CORRIGÉ : les commandes YouTube qui plantaient (pause, stop, next,
    prev, volume, shuffle, repeat, status, playlist, progress) sont maintenant
    disponibles car YouTubeMusicClient implémente toutes ces méthodes.
    """
...
Commandes disponibles :
  ...
  add [query]   → ajouter une musique à la playlist (sans interrompre la lecture)
  play / pause  → reprendre ou mettre en pause
  stop          → arrêter la lecture
  next          → piste suivante
  prev          → piste précédente
...
"""
    ...
    while True:
        ...
        if player_type == 'youtube':
            if cmd == "search":
                query = " ".join(cmd_full.split()[1:])  # garde la casse originale
                if not query:
                ...
                try:
                    ...
                    print(f"   Position dans la playlist : {len(player.get_playlist())}")
                except Exception as e:
                    print(f"⚠️ Erreur d'ajout : {e}")
                continue

        # Commandes communes (fonctionnent pour local ET youtube)
    ...
    while True:
        ...
        if choice in ("1", "local"):
            ...
            try:
                ...
                # BUG CORRIGÉ : utilise MUSIC_FOLDER au lieu d'une variable locale
